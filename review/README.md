...
Requires `rtmlib`, `imageio-ffmpeg`, `sam2` (install with `SAM2_BUILD_CUDA=0`; the custom CUDA kernel is not needed) and the usual torch/PIL/scikit-image stack. Model weights download on first run. `review/work/` is git-ignored; the sheets, run summaries and frame manifests are copied into `results/` so a set can be reviewed without regenerating it.

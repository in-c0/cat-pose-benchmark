...
`tail_anchored` was added after the owner's first look at the sheets found the propagated tail on a hind leg. See `detail/reports/anchored-tail-v0.md` for what it does and an eyeball comparison. `tail_grounded` (Grounding DINO + SAM2) was added 2026-09-14; see `detail/reports/grounded-tail-v0.md`. `tail_grounded_v1` (crop check + temporal) was added the same day; see `detail/reports/grounded-tail-v1.md`. `tail_grounded_v2` (v1 + SAM2 bridging) followed, and the body run switched to Grounding DINO as detector (33/33 and 22/24 frames with a cat); all sheets regenerated 2026-09-14. See `detail/reports/grounded-tail-v2.md`. The template now has 513 rows (nine parts).

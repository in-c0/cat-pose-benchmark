...
- **06 (20 Sep 2026):** [*Sixteen passes: what was proposed, what was tested, and what survived*](paper/06-sixteen-passes/README.md) ([PDF](paper/06-sixteen-passes/sixteen-passes-v0.1.pdf)). The audit trail behind note 05 as one table: sixteen proposals, each with its predeclared test, number and disposition, plus the four truth revisions and what found them. One survived (the two-sided bridge); every zero-shot semantic and single-feature structural verifier failed its test.
- **05 (20 Sep 2026):** [*Frozen truth: ten passes against fixed frame labels, and a prospective holdout that undid one of them*](paper/05-frozen-truth/README.md) ([PDF](paper/05-frozen-truth/frozen-truth-v0.2.pdf)). A written-down frame truth (147 dev frames, model-annotated, human pass pending) replaces the proxy. One change survives fifteen reviewer passes: a two-sided SAM2 bridge (v3), precision 0.953, recall 0.911 on the dev clips. Six ideas rejected on the same truth. Twelve holdout clips drawn by a precommitted metadata-only protocol and annotated before inference: the bridge's recall gain generalises on the second draw (0.73 → 0.77) and not the first; precision on unseen video is 0.69–0.73, set by wrong-part assertions the dev clips never showed.

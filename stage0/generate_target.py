...
The generated coordinates are construction coordinates, not measured physical truth.
After assembly, the target must be independently measured before any holdout points are
...
def create_board(face: dict[str, Any], dictionary):
    ...
    prototype = cv2.aruco.CharucoBoard(
        squares,
        square_length,
        marker_length,
        dictionary,
...
def save_board_png(
    ...
    square_length = float(face["square_length_m"])
    width_m = squares_x * square_length
    height_m = squares_y * square_length
    size_px = (
        metres_to_pixels(width_m, dpi),
        metres_to_pixels(height_m, dpi),
    )
...
def nominal_points(
    ...
    local_corners = np.asarray(board.getChessboardCorners(), dtype=float)

    points: list[dict[str, Any]] = []
    for local_id, local in enumerate(local_corners):
...
def main() -> None:
    ...
    parser.add_argument(
        "--spec",
        type=Path,
        default=Path(__file__).with_name("target_spec.json"),
    )
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--dpi", type=int)
    args = parser.parse_args()

    manifest = generate(args.spec, args.output_dir, args.dpi)

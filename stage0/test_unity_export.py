...
class UnityExportTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.config = json.loads((Path(__file__).with_name("layouts.json")).read_text(encoding="utf-8"))

    def test_committed_unity_scene_matches_exporter(self) -> None:
        expected = json.loads((self.root / "unity-viewer" / "Assets" / "StreamingAssets" / "stage0-scene-v2.json").read_text(encoding="utf-8"))
        self.assertEqual(compare_values(expected, export_scene(self.config, "A_symmetric_lateral")), [])
    ...
    def test_reflected_waypoints_lie_on_mirror_planes(self) -> None:
        ...
        for ray in scene["rays"]:
            if ray["kind"] != "reflected":
                continue
            mirror = mirrors[ray["name"].removeprefix("reflected-centre-ray-")]
            waypoint = ray["points_m"][1]
            point = np.asarray([waypoint["x"], waypoint["y"], waypoint["z"]])
            plane_point = np.asarray([mirror["plane_point_m"][axis] for axis in ("x", "y", "z")])
            normal = np.asarray([mirror["plane_normal"][axis] for axis in ("x", "y", "z")])
    ...
    def test_scene_uses_explicit_vector_objects(self) -> None:
        scene = export_scene(self.config, "A_symmetric_lateral")
        self.assertEqual(set(scene["representative_point_m"]), {"x", "y", "z"})

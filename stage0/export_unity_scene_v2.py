...
def export_scene(config: dict[str, Any], layout_name: str) -> dict[str, Any]:
    ...
    cameras = [
        {
            "name": "physical",
            "kind": "physical",
            "position_m": vec3(camera_centre),
            "look_at_m": vec3(camera["look_at_m"]),
        }
    ]
    rays = [
        {
            ...
            "kind": "direct",
            "points_m": [vec3(camera_centre), vec3(representative_point)],
        }
    ]
    ...
    for mirror in layout["mirrors"]:
        ...
        mirrors.append(
            {
                "name": mirror["name"],
                "plane_point_m": vec3(mirror["plane_point_m"]),
                "plane_normal": vec3(mirror["plane_normal"]),
                "vertical_axis": vec3(mirror["vertical_axis"]),
                "size_m": vec2(mirror["size_m"]),
            }
        )
        cameras.append(
            {
                ...
                "kind": "virtual",
                "position_m": vec3(virtual_centre),
                "look_at_m": vec3(representative_point),
            }
        )
        rays.append(
            {
                ...
                "points_m": [
                    vec3(camera_centre),
                    vec3(reflected_point),
                    vec3(representative_point),
                ],
            }
        )
    ...
    return {
        ...
        "capture_volume": {
            "min_m": vec3(config["capture_volume"]["min_m"]),
            "max_m": vec3(config["capture_volume"]["max_m"]),
        },

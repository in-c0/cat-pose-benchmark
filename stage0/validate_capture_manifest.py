...
def validate(manifest_path: Path, schema_path: Path, data_root: Path | None = None) -> list[str]:
    ...
    for item in manifest["files"]:
        ...
        if path.stat().st_size != item["byte_count"]:
            errors.append(f"{item['path']}: byte_count mismatch")
...
def main() -> None:
    ...
    parser.add_argument("--schema", type=Path, default=Path(__file__).parents[1] / "schemas" / "stage0-capture-manifest.schema.json")
